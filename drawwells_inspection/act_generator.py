...
def generate_drawwell_inspection_acts(elements):
    ...
    pdf.cell(30)
    pdf.multi_cell(100, 6, "АКТ\nобследования колодца", align="C", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(3)
    ...
    pdf.cell(165, 5, "Колодец № --" + elements['well'] + "-- название колодца --"+elements['name']+"--", align="L", new_y="NEXT", new_x="LMARGIN",markdown=True)
    pdf.multi_cell(165, 5, "Расположен по адресу: --"+elements['position']+"--", align="L", new_y="NEXT", new_x="LMARGIN",markdown=True)
    ...
    pdf.cell(165, 5, "**1. Координаты:** --" + elements['x']+" СШ    " +elements['y']+ " ВД--" , align="L", new_y="NEXT", new_x="LMARGIN", markdown=True)
    pdf.ln(2)
    pdf.cell(165, 5, "**2. Погодные условия:** --" + elements['weather']+"--" , align="L", new_y="NEXT", new_x="LMARGIN", markdown=True)
    pdf.ln(2)
    pdf.cell(165, 5, "**3. Характер использования колодца:** --" + elements['using']+"--" , align="L", new_y="NEXT", new_x="LMARGIN", markdown=True)
    pdf.ln(2)
    pdf.cell(165, 5, "**4. Местоположение в рельефе:** --" + elements['geomorph']+"--" , align="L", new_y="NEXT", new_x="LMARGIN", markdown=True)
    pdf.ln(2)
    pdf.cell(165, 5, "**5. Название и индекс водоносного горизонта:** --" + elements['aquifer']+"--" , align="L", new_y="NEXT", new_x="LMARGIN", markdown=True)
    pdf.ln(2)
    pdf.cell(165, 5, "**6. Водовмещающие породы:** --" + elements['lithology']+"--" , align="L", new_y="NEXT", new_x="LMARGIN", markdown=True)
    pdf.ln(2)
    pdf.cell(165, 5, "**7. Характеристика колодца:**", align="L", new_y="NEXT", new_x="LMARGIN", markdown=True)
    pdf.set_font("TNRoman", size=10)
    col_width = pdf.epw / 2 # distribute content evenly
    for row in elements['condition_data']:
        if (len(row[1])*3/165)==0:
            height=pdf.font_size*1.8
        else:
            height=pdf.font_size*(int((len(row[1])*3/165))+2)
        for datum in row:
    ...
    pdf.set_font("TNRoman", size=11)
    pdf.cell(25, 5, "**8. Примечания:**", align="L", new_y="NEXT", new_x="LMARGIN", markdown=True)
    pdf.multi_cell(165, 5, elements['comments'], new_y="NEXT", new_x="LMARGIN", markdown=True)
    pdf.ln(2)
    pdf.cell(25, 5, "**9. Рекомендации:**", align="L", new_y="NEXT", new_x="LMARGIN", markdown=True)
    pdf.multi_cell(165, 5, elements['recommendations'], new_y="NEXT", new_x="LMARGIN", markdown=True)
    pdf.ln(2)

    pdf.cell(25, 5, "**10. Приложения:**", align="L", new_y="NEXT", new_x="LMARGIN", markdown=True)
    pdf.cell(25, 5, "1. Схема расположения колодца", align="L", new_y="NEXT", new_x="LMARGIN")
    ...
    # pdf.set_font("TNRoman", "B", 14)
    # pdf.cell(30)
    # pdf.multi_cell(100, 6, "Схема расположения колодца\n№ "+elements['well'], align="C",new_y="NEXT",new_x="LMARGIN")
    ...
    pdf.set_font("TNRoman", "B", 14)
    pdf.cell(30)
    pdf.multi_cell(100, 6, "Фотодокументация по колодцу\n№ "+elements['well'], align="C",new_y="NEXT",new_x="LMARGIN")
